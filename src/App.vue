  ...
  <div id="app" @keyup="handleKeyUp">
    <div class="main-container">
      <count-down
        ...
        @timeup="handleTimeup"
      />
      <streak ref="streak" :streak="streakCount" :result="result" />
      <div class="option-container">
        <button class="option-btn" @click="handleClickModeBtn('free')">
          FREE MODE
        </button>
        <button class="option-btn" @click="handleClickModeBtn('countdown')">
          COUNT DOWN MODE
        </button>
      </div>
  ...
  setup() {
    ...
    const startNewQuiz = () => {
      const newHangul = pick(
        Object.entries(hangul.double).filter(
          (item) => item[0] !== currentQuiz.spell,
        ),
      );
    ...
    const checkAnswer = (answer) => {
      ...
      if (correct) {
        const answerTime = (
          (new Date().getTime() - currentQuiz.startTime) /
          1000
        ).toFixed(2);
        ...
        if (quiz.value && !quiz.value.answerShown) {
          streakCount.value++;
          if (streakRef.value) {
            streakRef.value.playComboAnime();
          }
        }
        setTimeout(() => {
          startNewQuiz();
    ...
    const setQuizPool = () => {
      ...
      for (let time = 0; time < defRatio; time++) {
        quizPool.value = quizPool.value.concat([
          ...Object.entries(hangul.double),
        ]);
      }
    };

    const decreaseRate = () => {
      const itemsInPool = quizPool.value.filter(
        (item) => item[0] === currentQuiz.spell,
      ).length;
      if (itemsInPool > 1) {
        const indexOfItemToDel = quizPool.value.findIndex(
          (item) => item[0] === currentQuiz.spell,
        );
    ...
    const handleTimeup = () => {
      ...
      newResult.correctPer = (
        (newResult.correct * 100) /
